...
pub use crate::packet::{
    ConnectComm, CoptFrame, DtData, Parameter,
    PduType, TpduSize,
};
...
impl<
        F: Debug + Eq + PartialEq,
        E: Encoder<F>,
    > Encoder<CoptFrame<F>> for CoptEncoder<E>
where
    <E as Encoder<F>>::Error:
        ToCoptError + Send + Sync + 'static,
{
    type Error = Error;
    ...
    ) -> std::result::Result<(), Self::Error>
    {
        ...
        match item.pdu_type {
            ...
            PduType::DtData(conn) => {
                dst.put_u8(0xf0);
                let merge = conn.tpdu_number >> 1
                    | if conn.last_data_unit {
                        0b1000_0000
                    } else {
                        0
                    };
    ...
    > {
        ...
        match *pdu_type {
            ...
            0xf0 => {
                ...
                let sub_length =
                    pre_length - sub_src.len();
                let mut src = src
                    .split_to(length + sub_length)
                    .split_off(2);
