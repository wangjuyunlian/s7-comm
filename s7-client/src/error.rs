    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            S7ConnectError::Response { code } => {
                write!(f, "Error response: {}", error_text(*code))
            }
            S7ConnectError::CPU { code } => {
                write!(f, "Error response CPU: {}", error_text(cpu_error(*code)))
            }
            S7ConnectError::InvalidInput { input } => write!(f, "Invalid input: {}", input),
            S7ConnectError::Send => {
            ...
            S7ConnectError::TryFrom(bytes, reason) => {
                write!(
                    f,
            ...
            S7ConnectError::InvalidCpuStatus(status) => write!(f, "Invalid cpu status {}", status),
            S7ConnectError::InvalidResponse { reason, bytes } => {
                write!(
                    f,
