...
impl<F, E: Encoder<F>> Encoder<TpktFrame<F>>
    for TpktEncoder<E>
where
        ...
        ToTpktError + Send + Sync + 'static,
{
    ...
    fn encode(
        ...
        self.0
            .encode(item.payload, &mut payload)?;
...
impl<F, D: Decoder<Item = F>> Decoder
    for TpktDecoder<D>
where
    <D as Decoder>::Error:
        ToTpktError + Send + Sync + 'static,
